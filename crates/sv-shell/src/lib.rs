...
// 矢量动画(Lottie):feature `lottie`。资产类型 + 注册 API 随之进出。
// 下游用 `Lottie::from_slice` 解析后交给 `register_vector`
#[cfg(feature = "lottie")]
pub use animation::{register_vector, unregister_vector};
#[cfg(feature = "lottie")]
pub use sv_lottie::{Error as LottieError, Lottie};
// PAG 位图序列:feature `pag`。下游用 `sv_pag::PagFile::parse` 拿 `BitmapSequence`,
// 配一个图片解码器交给 `register_pag`(差分帧重放成逐帧图)
#[cfg(feature = "pag")]
pub use animation::{register_pag, register_pag_webp};
#[cfg(feature = "pag")]
