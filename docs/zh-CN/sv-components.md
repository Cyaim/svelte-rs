...
一个 `.svelte` 文件最多三个块:`<script>`(纯 Rust + runes,必须是第一个块)、模板标记(Svelte 模板语法 + 封闭元素集)、可选的 `<style>` 块(scoped 类,习惯放最后)。元素集是封闭的:`<view>`、`<text>`、`<button>`、`<checkbox>`(叶子元素,必须自闭合)。未知小写标签直接编译报错;首字母大写的标签(如 `<TodoItem />`)是组件调用。
...
每个 `.svelte` 文件编译成一份人类可读的 Rust 源码(`prettyplease` 格式化),导出 `pub fn <fn_name>(doc, parent[, props])`;函数名取文件名 snake_case 化(`Counter.svelte` → `counter`,`TodoItem.svelte` → `todo_item`)。编译错误带 `.svelte` 源文件内 1-based 的行/列定位。完整可运行示例:`examples/counter-sfc`、`examples/todo-sfc`、`examples/showcase`。
...
**keyed 行的绑定是反应式的**(ADR-7):行内 `it` 实际是该行的 `Signal<T>`,
所以 key 不变、内容变了的行**原地更新**,不重建、不丢行内状态;顺序没变时
连树都不动。代价是绑定名必须是**单个标识符**(解构请在行内用 `{@const}`),
且项类型需要 `Clone + PartialEq`。
非 keyed `{#each}` 不变:仍是整块重建,绑定名是普通值。

⚠️ **组件 prop 是快照**:`<TaskRow label={it.1} />` 在行构建时取一次值。
行内直接写的标记(`<text>{it.1}</text>`)会随内容更新,传进子组件的 prop
不会——要联动请传信号(见 `$bindable`)。
...
元素级 `bind:` 支持 `<checkbox>` 的 `bind:checked`、`<input>`/`<textarea>` 的 `bind:value`、滚动容器的 `bind:scrolly`。其余 Svelte 目标(`bind:this`、尺寸类、媒体类)写了会编译报错并指向支持矩阵。
...
组件标签用 PascalCase,解析到同名 `.svelte` 文件 snake_case 化的函数——`.svelte` 文件内不需要任何 import,因为 `sv_compiler::build` 会先整体扫描源码目录、第一遍注册全部 `$props` 签名(组件可以互相引用);生成的所有 `.rs` 文件随后要 `include!` 进同一个 Rust 作用域(见下文)。
...
`sv_compiler::build("src")` 递归收集 `*.svelte`,先注册全部 `$props` 签名,再逐个编译到 `$OUT_DIR/<fn_name>.rs`,并逐文件发出 `cargo::rerun-if-changed`;编译失败直接 panic,格式 `文件:行:列: 消息`。多组件应用把每个生成文件都 `include!` 进同一作用域——例如 `examples/todo-sfc/src/main.rs` 同时 include 了 `todo.rs` 与 `todo_item.rs`。
...
## `sv check`:让 rustc 的报错指回 `.svelte`

`.svelte` 编译成 `$OUT_DIR` 里的 `.rs`,而 **rustc 没有 `#line` 指令** —— 类型错误
...
- 覆盖率约 **80%**(10 个 `.svelte`,281/349 个用户写的 Rust token 拿到精确映射)。
- 映射不到时**绝不吞掉诊断**——原样输出生成文件的位置,并附一句为什么没映射上
  (共五种理由:没有 map、map 损坏、map 过期、`.svelte` 找不到、锚点表熔断)。
  吞掉一条诊断比不映射糟得多。
...
样式域、模板域的错误由编译器自报,本来就带 `.svelte` 行列,不经过这条路。
