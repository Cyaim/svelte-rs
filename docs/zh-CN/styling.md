...
CSS 嵌套只支持一种形态:规则内的 `&:hover` / `&:active` / `&:focus`。独立写法 `.btn:hover { }` 等同样可用。其它伪类(如 `:disabled`)是编译错误,随禁用态排期。
...
运行时没有任何选择器匹配。对每个带 `:hover` 规则的元素,编译器生成一个私有布尔信号,接到该元素的 pointer-enter/leave 回调上;`:active` 是第二个状态位,接 pointer-down/up;`:focus` 是第三个,接**焦点链**(不是指针)。整个元素的样式变成一个响应式闭包(`bind_style`):基础声明 → focus → hover → active,对齐 CSS 的 L-V-F-H-A 顺序(按压态最终生效)。你自己写的 `onpointerenter`/`onpointerleave`、`onfocus`/`onblur` 都会与内部接线**合成**,不会被覆盖——sv-ui 每种回调只有一个槽位,分开设会互相顶掉。

写了 `:focus` 的元素会被**自动设为可获焦**(与 `onkeydown` 同理):不自动设位,样式永远不生效而且查不出原因。
...
```
元素规则 < 类(按 class="a b" 顺序) < 内联 style="" < class: 条件类 < :focus < :hover < :active < style: 指令
```
...
| ⏳ 等基建 | 17 | 渐变/阴影/滤镜(vello 后端)、字体(parley)…(滚动已于 R2 落地:`overflow` / `overflow-x` / `overflow-y` + 滚轮/滚动条拖拽/裁剪;简写写两轴,分轴可做"横向裁掉、纵向滚") |
