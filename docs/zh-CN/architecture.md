...
├──────────────────────────────────────────────────────────────┤
│ sv-macro     view! 过程宏前端(只剩 parser)                   │
│ sv-compiler  .svelte 前端 + 公共模板 IR / 共享 codegen         │
│              → 两个 parser 汇入同一内核,编译成对 sv-ui        │
│                绑定原语的调用                                  │
├──────────────────────────────────────────────────────────────┤
...
开窗路径是**帧对齐**的(ADR-6,已落地):写 signal 只入队 + 催一帧,
渲染壳在帧首统一冲刷;要立刻看到结果调 `sv_reactive::tick()`(flush 逃生舱)。
离屏渲染与测试路径无调度器,写入仍同步 flush,行为不变。

## 双编译前端,同一编译目标(ADR-2 修订版)

两个前端编译产物相同:命令式的 `Doc` 建树代码 + 对 sv-ui 绑定原语的调用。
两条路线的生成代码形态几乎一致——这正是合并内核成立的依据,内核合并如今已完成,
两个前端只剩各自的 parser(见下)。
...
| 模板语法 | 受 Rust tokenizer 约束(文本要引号,Rust 的 `if`/`for`) | 原汁 Svelte:免引号文本、`{#if}{:else}`、`onclick`、`bind:` |
...
**① 已完成,分两步(2026-07-22 发射口收敛;2026-07-23 内核合并落地)**:
对 sv-ui 的**发射口收敛为一处**
`sv_compiler::emit`(绑定原语调用词汇表 + 重建闭包协议);公共模板 IR 落在
`sv_compiler::template`,codegen 只有一份(`sv_compiler::generate_template`
是宏侧入口)。两个前端如今**只剩各自的 parser**,原语签名或 codegen 变更
从此只改一处。span 精度没有赔进去:IR 的表达式载荷是双态的(`ExprSrc`)——
`.svelte` 携带带字节偏移的源码文本(过 runes 改写),`view!` 携带带真 span 的
Rust token(原样直通,不过 runes 改写)。`.svelte` 路线最大的
悬置风险仍是 IDE 体验(`.svelte` 内没有 rust-analyzer,Volar 式转发 LSP 未 spike)。
...
- 与 Svelte 的刻意差异:effect 创建时同步首跑,而非推迟到微任务;开窗后的
  后续写入按帧对齐攒批(ADR-6,已落地)。
...
| `sv-macro` | `view!` 过程宏前端:parser,产出 sv-compiler 的公共模板 IR(codegen 亦共享) |
| `sv-compiler` | `.svelte` SFC 前端 + 双前端公共内核(模板 IR `sv_compiler::template`、共享 codegen):runes 源变换、Svelte 模板语法、样式解析、`build.rs`/`OUT_DIR` 集成(`sv_compiler::build`),错误带 `.svelte` 行列 |
...
| ADR-2(修订) | 编译策略:双前端(`view!` + `.svelte`)共享同一编译目标;M1 合并为单一编译器核心 | 已实现——内核合并完成(2026-07-23),双前端只剩 parser 分叉 |
...
| ADR-6 | 帧调度:写入攒批进帧管线,配 `tick()` 逃生舱 | 已落地(2026-07-22)——开窗写入攒到帧边界统一冲刷 |
