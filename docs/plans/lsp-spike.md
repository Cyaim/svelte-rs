...
5. **第一步不是 LSP,是把编译器域与 rustc 域的错误都变成 `.sv` 坐标**。理由不是保守,
   是杠杆:今天 `.sv` 用户写错一个类型,错误落在
   `target/debug/build/<pkg>-<hash>/out/counter.rs:44:44`(复核实测的真实坐标);
   写错一个模板/CSS,错误埋在 build.rs 的 **panic dump** 里。两条都要搬回
   `Counter.sv:12:38`,是整条路线上单位成本体感最高的一步,100% 在我们自己的代码里,
   可被 `cargo test` 验收。
...
3. **`prettyplease` 不是恒等打印**(实测 M1):`quote!{ g(a, b,); }` 的尾逗号被吃掉,
   ...
   **复核修正(M1 的结论对,但取样不代表真实产物)**:73 vs 71 是手写 `quote!` 里
   人为放尾逗号造出来的。在**本仓库 11 个真实 `.sv` 的生成产物**上重跑
...
**规模**:8 个 `.sv`(Counter/InputDemo/Dialog/Settings/Stepper/TaskRow/TodoItem/wide)
的生成产物里,`.get()`/`.set()`/`.update()` 形态的用户变量引用共 **98 处**,
**全部丢失 provenance**。Counter.sv 一个文件就有 9 处(生成行 7/9/44/46/92/117/137/142/160),
而 Counter.sv 里用户写的 `count`/`double` 引用**总共就这么多**。
...
- "下一个 `__elN` 的首次出现即终点"这个启发式在真实产物上**会错**。实测
  Counter.sv 的生成文件:`__el7`(归零按钮)首现于 L120,`__el8` 首现于 L145,
...
记录"当前节点的 .sv 区间",走到的每个 pre-token 都带上栈顶;并行走给出该 token 在
...
2. **样式没有 provenance**。样式值在编译期就折叠成字面量——实测 Counter.sv 的产物里
   确实是 `s.bg = Some(::sv_ui::Color::rgba(255u8, 62u8, 0u8, 255u8));`,
   `s.font_size = 20f32;`,全是合成 token。
   (复核更正:这不止发生在 `<style>` 块,**内联 `style="..."` 属性同样折叠**,
   而本仓库示例用内联样式远多于 `<style>` 块——Counter.sv 根本没有 `<style>` 块。)
   **这没关系**:样式域的错误本来就由编译器自报 .sv 行列,rustc 永远看不到它们。
...
4. **(复核新增,而且这条打到了本方案自己的招牌 demo)主 span 经常落在标点上,
   而标点不在 `tokens[]` 里**。实测:把 §6.1 的示例错误注入真实 `Counter.sv`
   ...
   **报不出 §6.1 承诺的 `Counter.sv:12:38`**。
   **补法(必须写进 P0)**:查表未命中 `tokens[]` 时,先做**相邻锚点插值**——
   取生成侧 gen_start 左右最近的两个 token 段,若二者的 .sv 区间同属一个表达式且
   顺序一致,则按"到左锚点的字节距离"平移出 .sv 位置。对 `count.get() + "x"` 这种
   `左锚点=count`(第零步修好之后才有)、`右锚点="x"` 的情形,`+` 能被精确插值回
   `.sv` 的 `+`。**这条与第零步是绑定的:第零步不做,左锚点就是胶水,插值也救不回来。**
...
`.sv` 的模板/CSS/runes 错误走 `lib.rs:207` 的 `panic!`,实测(注入 `fg="#zzz"`)结果是:
...
```
error: svdemo@0.0.0: src/Counter.sv:19:15: 属性 `fg`:颜色 `#zzz` 不是合法十六进制
...
| **build.rs 失败(= `.sv` 编译器域错误)** | 见 §4.1 第 0 分支 | 本包无 `build-script-executed`;从 stderr 抓 `<sv>:<行>:<列>: <消息>` 输出为一等诊断。**这条最常见**——用户改 `.sv` 时语法错的频率远高于类型错 |
...
- 工作量:**1–2 人周**(前提:§3 的 map 已就位)。零 r-a 实例、零虚拟文档、零 LSP server、
  零 crate graph 改动。
- 收益(**复核下修**):`.sv` 文件里出现波浪线与 Problems 面板条目。但初稿写的
...
- **落盘不是必需的(复核实测,M8c)**:r-a 对 `include!(concat!(env!("OUT_DIR"),…))`
  ...
  **不落盘就已经能用**。所以初稿"a1 = 落盘 + 转发"里的"落盘"应当拆出来单算:
  - **a1a(推荐先做)**:不落盘,直接对 OUT_DIR 的 URI 做正/反向转发。
    goto 目标落在 OUT_DIR 时,用 `.map` 反向改指 `.sv` —— 用户压根看不到那个丑路径,
    **也就不需要为了"体面"而落盘**。
...
```
error: counter-sfc@0.1.0: src/Counter.sv:19:15: 属性 `fg`:颜色 `#zzz` 不是合法十六进制
```

(两段都是复核实测输出。)配一份 `.vscode/tasks.json` problemMatcher,
`.sv` 的**编译器域**错误当天就进 Problems 面板 + 编辑器波浪线,**一行映射代码都不用写**。
...
`.sv` 侧 `12:38` 的坐标经核对是对的(`+` 在 Counter.sv 第 12 行第 38 个字符)。
...
不做的话,输出只能退到 `Counter.sv:12`(元素级),说服力大打折扣。
...
1b. **完整性(completeness)断言 —— 复核新增,这才是 P0 真正的验收闸**:
   - `map_covers_all_reactive_reads`:对每个 `.sv`,统计 script.vars 里每个反应式变量
     在 `.sv` 源码中的引用次数 N,断言 `tokens[]` 里映射到这些引用位置的段数 == N。
     (今天这个数是 **0/98**;第零步做完应当是 98/98。)
   - `map_covers_each_pattern_and_bind`:`{#each x as pat}` 的 `pat`、`bind:value={x}`
     的 `x`、`$props` 字段类型,各至少有一段映射。
   - **覆盖率地板**:对全部 `.sv`,`.sv` 侧"落在模板表达式与 script 块内的非空白字节"
...
- 于是本条的止损("退到保存时才重生成")**其实是唯一可选项,不是退路**。
  实测保存路径的墙钟:改一次 `.sv` → `cargo check` **242–261 ms**(小工程,§1.2),
...
M13(真实 `Counter.sv` 注错的端到端坐标)、M14(`format_ident!` 的 provenance 丢失)、
M15(`.sv` 保存 → `cargo check` 墙钟)、M16(proc-macro2 `byte_range` 语义源码核实)。
...
- **真实 app 规模下的 `.sv` 保存 → cargo check 墙钟**。复核实测的 242–261 ms 来自
...
- **ADR-2(双前端共存)**:本方案不改变 ADR-2,反而给它加了一条实证——
  `view!` 宏路径的 span 精度是免费的,`.sv` 路径的 span 精度要花 **7.4–13.4 人周**买
...
**真实的 Counter.sv** 跑 `cargo check --message-format=json`,得到的是
...
| §3.4.2 | 只说 `<style>` 块 | 内联 `style="…"` 同样折叠成 `Color::rgba(255u8, 62u8, 0u8, 255u8)`,而 Counter.sv 根本没有 `<style>` 块 |
...
1. **编译器域错误根本不进 cargo 的 JSON 流**(§4.1 新增第 0 分支)。`.sv` 的模板/CSS/runes
...
2. **`build()` 的 `panic!` → `println!("cargo::error=…")`,列为 P-1,0.2–0.4 人周**(§4.1)。
   实测输出 `error: svdemo@0.0.0: src/Counter.sv:19:15: 属性 …` + `error: build script
...
- **`prettyplease` 未来版本的行为**:复核只证明了 0.2.37 在**当前 11 个 `.sv` 的产物形状上**
