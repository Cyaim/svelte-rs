...
| 7 | 现在该不该做 | ~~步骤 0 该做~~ **已做完**;lottie 本体仍排在改名 ADR-10、`.svelte` LSP spike、增量布局/脏矩形之后 | 见 §8.3 |
...
| 3 降级 + a11y + 双前端 + 双语文档 | **1.5–2.5** | **中低** | 双语文档与支持子集表是实打实的时间;`.svelte` 元素属性表 + 编译期错误信息按 `<overlay>` 那次的经验约 0.5 人周 |
...
> ⚠️ **复核:"前一半"已经在本文入库前 6 分钟做完了(§0/§2/§7 复核)。**
> 这段论证现在只有存档价值。**新的"前一半"由复核给出,见 §8.4。**
...
- 0.5–1 人周,高置信度。这是本仓库当前少见的"高收益、低风险、无耦合"条目。
  ——**事后验证:这个估算是准的**,`7966785` + `3ebe81c` 两个 commit 落地,
  改动集中在 `paint.rs` + `vello_backend.rs`,7 条测试。**本文对这一段的判断
  被现实证实了;被证伪的只是"还没做"这个前提。**
...
| `.svelte` 的 LSP spike | 风险清单**第 1 条**,是 `.svelte` 前端能否转正的悬置条件。悬置一天,双前端策略就多背一天的债 |
| 增量布局 | ~~30k 全量档实测 130–160ms,**已越过预案里 2ms 的触发线**(DESIGN R2 记录)~~ **⚠️ 复核改写**:该读数已被 DESIGN.md 当天自己作废(探针让 30k 叶子共享两种文本串,measure 缓存永远命中),真实变更帧是 **311ms**,`caea14c` 已压到 **111ms**;而 `incremental-layout.md` §7.1 明说「**2ms 这条线对全量档没有工程意义**」。结论方向不变(仍排在 lottie 前),但**不能再拿 2ms 触发线当理由** |
| **脏矩形 / 局部重绘(复核补入)** | **这是 lottie 的硬前置,不是并列项。** 30k 树纯绘制帧实测 **117ms**(`incremental-layout.md` §8.2),而 lottie 一跑就是整窗每帧重绘。没有它,§4.2 那套"不 bump"精算救不回来。`lottie-1-ecology.md` §7.2 把它排在自己清单的第二位,理由同 |
...
> ⚠️ **复核:这个判据成立,复核完全同意,但"130ms"要换成"311→111ms,
> 且纯绘制帧还有 117ms"。** 数字换了之后结论只会更硬,不会更软。
> 另外补一条本文没写的判据:**lottie 目前会 panic**(§5.3 复核)。
> 一个尚未发布的库,不该在发布前把"第三方数据能崩进程"这个面积扩大。
...
### 8.4 复核补节:本文漏掉的四条"20% 力气 80% 收益"

本文把选项空间收成了"做 lottie / 不做 lottie",但触发条件 2 里自己承认,
真正的需求来自 **sv-arco 的 Spin / Skeleton / Progress / Result / Empty**。
**这批需求有四条更便宜的路,本文一条都没评估。**

**① 程序化动效组件(推荐,收益/成本比最高)。**
路径动词已经在了(§2 复核)。arco 那批动效——转圈 Spin、脉冲 Skeleton、
环形 Progress、勾选 Result——**全都是可以直接用 `fill_path`/`stroke_path` +
一条时间参数画出来的**,不需要 lottie、不需要 velato、不需要 serde、
不碰 MSRV、不新增 panic 面、也不需要资产管线。
一个转圈 spinner 是"一段圆弧,起止角随时间走",几十行。
**代价**:每个动效要手写一次,设计侧不能用 AE 出稿。
**但这恰恰是 Flutter 的做法** —— `lottie-1-ecology.md` §4.1 的原话:
"有 Skia 也不用 Skottie,纯 Dart 自绘"。
**判据**:如果需求是"库自带的 5–8 个动效组件",走这条;
如果需求是"用户能塞任意设计师文件",才需要 lottie。**这两件事被本文合并成了一件。**

**② 只做 SVG 静态图标(build 期 usvg → `PathCmd` 表)。**
本文 §2.7 提到了,但当成 lottie 的"顺带兑现"。**顺序应该反过来**:
它才是调研 26 点名的头号卡点的**剩下那一半**,与 lottie 完全解耦,
零运行时依赖,直接解锁 A1/A2 波次的图标回填。

**③ 构建期把 lottie 预渲染成帧数据(本文 §7 步骤 4 只提了运行期缓存)。**
本文的"预渲染缓存"是**运行期**把 N 帧命令流缓存下来 —— 那不省依赖。
**真正的 80/20 是把 velato 挪到 build 期**:build.rs / 一个离线工具跑一次
`Composition` 求值,吐出 `&'static [(帧号, &[PathCmd], Color)]` 的数据表。
这一刀同时砍掉 §8.2 的**四项隐藏成本**:运行期 panic 面(②)、MSRV 耦合(③)、
默认构建 15 个新 crate(④)、以及每帧 `append` 的编码开销。
**代价**:帧数 × 路径数据的体积(24px/60 帧的 spinner 很小,全屏插画会爆),
且失去运行期 seek/变速。
**注意**:这与 `lottie-1-ecology.md` §7.3(c) 否掉的"转译成 `.svelte`/`view!` **代码**"
**不是同一件事** —— 那条被否是因为"Qt 能这么干是因为 QML 底下有通用属性动画系统,
我们的 `anim.rs` 只有两个通道"。而**转译成数据表不需要通用动画机**:
播放器就是"按 `time_ms` 查表 + 发命令",正是 ADR-2 的"生成数据而非类型"。
**复核认为这条被兄弟文档连坐否掉了,值得单独重估。**

**④ 先只支持静态首帧(本文的步骤 1 独立发布)。**
空状态插画、Result 成功图、引导图——**这批需求里有相当一部分根本不需要动**。
步骤 1 单独合入就能吃掉它们,而步骤 2 的全部风险(不 bump、丢最后一帧、
a11y 空转、整窗重绘)一条都不用背。**本文把步骤 1 定义成"步骤 2 的前置",
没把它定义成"一个可以独立发布的能力"** —— 而它明明满足本文自己
在 §7 开头立的规矩("能单独合入、单独有价值")。
...
> ⚠️ ### 复核:第 3、4、6 条已有答案,不该继续挂在"未核实"里
>
> - **第 3 条(编译时间)→ 有方向性答案。** `lottie-1-ecology.md` §6.1 实测
>   `velato 0.11 (default-features=false)` 的依赖树 **15 个 crate**,
>   「全量编译(冷启动、release)……**秒级**」。仍建议在 PR 里附
>   `--timings`,但"这是不是个编译时间问题"已经可以答:**不是**。
> - **第 4 条(每帧 `append` 开销)→ 已实测,而且结论与本文的担心相反。**
>   本文说"这个数决定 lottie 能不能和 ADR-9 的百万控件目标共存"。
>   `lottie-1-ecology.md` §6.2 的数(velato 官方 Tiger.json,419KB / 14 图层,
>   120 帧扫描均值,release):**场景构建 0.007 ms/帧**,解析 19.3ms(一次性)。
>   含 tiny-skia 光栅的整帧:128px 0.55ms / 256px 0.99ms / 512px 2.24ms /
>   1024px 6.18ms。
>   **也就是说 velato 的编码开销可以忽略不计,成本全在我们自己的光栅上。**
>   连带两个结论:①步骤 4 的"命令流缓存"**不必要**(它缓存的正是那 7µs);
>   ②真正吃预算的是输出尺寸,图标档(≤256px)在纯 CPU 上近乎免费。
>   **本文把第 4 条列为"最关键的未核实项"是对的,但它在同批入库的文档里
>   已经有答案了 —— 两份文档没有互相引用,这是本轮最该修的协作问题。**
> - **第 6 条(MSRV 构建道)→ 30 秒可查,答案是"一定会开"。**
>   `.github/workflows/ci.yml:219–242`,msrv job 跑
>   `cargo build --workspace --all-features`。见 §8.2 复核。
>
> **仍然成立的未核实项**:第 1、2、5、7 条。复核补充:
> - 第 2 条复核也只查到 `fixed::Brush = peniko::Brush`、
>   `fixed::ColorStops = peniko::ColorStops`(velato `src/runtime/model/fixed.rs`
>   逐字),`Gradient` 的 `stops` 内部结构**仍未逐字核实**。
> - **补第 8 条:GPU(`backend-vello`)路径的实机帧时间未测。**
>   §6.2 那组数只覆盖 CPU。本文 §5.4 的 parity 容差 `[0.95, 1.05]` 也是
>   拍脑袋的,没有任何 lottie 样本的实测支撑(既有的 1.001/1.017 是字形与矩形的)。
> - **补第 9 条:`push_layer` 的忽略策略在"matte 用在关键位置"的文件上退化成什么样,
>   未测。** §6.3 只在 Tiger.json 上验过"退化得不难看",作者自己声明
>   「这不是正确性证明」。

---

## 10. 复核记录(2026-07-22,对抗性复核)

**立场**:默认这份产物有问题,逐条自查,不信转述。
**权限**:只改 `docs/plans/`,未动任何代码。**未 commit、未 add、未切分支。**

### 10.1 核实通过的(先说好的,免得后面像一边倒)

上游事实这一块**扎实,复核没找到任何编造**。逐条重查(2026-07-22):

| 断言 | 复核方式 | 结果 |
|---|---|---|
| velato **0.11.0**,发布 **2026-07-21** | `crates.io/api/v1/crates/velato` | ✅ 逐字一致(14 个版本,0.1.1 yanked) |
| `pushed_at` 2026-07-21 / **152** star / **6** open issue / 未 archive | `api.github.com/repos/linebender/velato` | ✅ 四项全中 |
| 许可 `Apache-2.0 OR MIT` | crates.io + GitHub `license.spdx_id` | ✅ |
| MSRV **1.88**,且"抬 MSRV 不算 breaking" | README 原文 | ✅ 引文逐字无误 |
| velato 0.11 ↔ vello **0.9** | README 兼容表 | ✅ 表中 `main, 0.11 → 0.9` |
| `vello ^0.9` **optional**;kurbo/peniko/serde/serde_json/serde_repr required | `/crates/velato/0.11.0/dependencies` | ✅ 版本约束逐字一致 |
| `default-features = false` 能关掉 vello | velato `Cargo.toml`:`default = ["vello"]` / `vello = ["dep:vello"]` | ✅ |
| `RenderSink`/`Renderer`/`Composition` **不受 feature 门控** | velato `src/lib.rs` | ✅ 只有 `pub use vello;` 带 cfg |
| `RenderSink` 六个方法签名 | `src/runtime/render.rs` 原文 | ✅ **逐字一致**(含两个默认空实现) |
| `Renderer::append` 签名、`Composition` 六个公开字段与三个构造 | 同上 + `runtime/mod.rs` | ✅ |
| `fixed::Brush = peniko::Brush` / `fixed::Stroke = kurbo::Stroke` | `runtime/model/fixed.rs` | ✅ |
| "alpha 由 velato 乘进 brush" | `render.rs` 里 `draw.brush.clone().multiply_alpha(..)` | ✅ **本文这条细节是对的**,不是想当然 |
| `Mask::new / fill_path / intersect_path` 在 tiny-skia 0.11.4 存在 | docs.rs | ✅ 签名逐字一致 |
| accesskit 0.24 有 `Image`/`Canvas`/`GraphicsObject`/`SvgRoot` | 本地 registry `accesskit-0.24.1/src/lib.rs` | ✅ 行号 68/125/215/193 |
| WCAG 2.2.2 **Level A** + "more than five seconds" | w3.org/WAI/WCAG22 原页 | ✅ 逐字(但本文**省了 essential 例外从句**,见 §6 复核) |
| rlottie **0.5.4**(2026-03-07,codeberg,C++)/ rasterlottie **0.2.1**(2026-04-24,下载三位数) | crates.io API | ✅ |
| 本仓库 `vello 0.9` / `kurbo 0.13.1` / `peniko 0.6.1` / `rust-version 1.88` / `tiny-skia 0.11` | `Cargo.toml` + `Cargo.lock` | ✅ |
| 调研 26 的两处引文("上限约六成"、"图标以 stroke 为主") | `docs/research/26-*.md:217/293` | ✅ |
| DESIGN.md 风险清单第 1/4/5 条、parity 1.001/1.017 | `docs/DESIGN.md` | ✅ |
| §4.4 的 vello 短路 bug | `vello_backend.rs:299` `if !scene_unchanged { .. }` | ✅ **真 bug,分析正确,是本文最有价值的发现** |
| §3.3 的 `unreachable!("View 不是叶子")` | `render.rs:357` | ✅ |
| `build_taffy` 叶子判定 / `layout_full_cached` / `paint_tree` 的 View 分支行号 | `render.rs:279/458/923` | ✅ 三处全中 |

**没有找到一处编造的版本号或 API 名。** §1 的"没有一处是凭记忆写的"这句话,
复核认可。
...
| # | 位置 | 问题 | 严重度 |
|---|---|---|---|
| 1 | 卷首 / §0 / §2 / §7 / §8.1 / §8.3 | **前提整体过期**:`fill_path`+`stroke_path` 已在 `7966785`/`3ebe81c` 入库,比本文入库早 6 分钟;本文最强的结论("步骤 0 现在就该做")已被兑现 | **致命** |
| 2 | §4.2 | "不 bump 就便宜"量级错误:静止帧(缓存全命中)仍 117ms、命中仍 clone 1.4MB、CPU 每帧调两次;引的 130–160ms 是 DESIGN.md 当天自己作废的读数 | **致命** |
| 3 | §5.3 / §8.2 | velato 在**合法**输入上 panic(非 `Err`),`Error` 只有 `Json` 一个变体;本文的"加载期扫描"物理上做不到,且撞 R4 红线 | **致命** |
| 4 | §6 第 5 条 | a11y 断言与源码相反:`push_access_tree`(`lib.rs:318`)每个非短路帧都跑,`animating` 正是绕开短路的条件 | **高** |
| 5 | §4.4b(新增) | 同一个"不 bump"会让**非循环动画的最后一帧在两个后端都被丢掉**;本文只找到 vello 那一半 | **高** |
| 6 | §5.2 | mask 与 matte 混为一谈;matte 忽略后**蒙版源被当可见内容画出来**;"split positions 不支持"已过期 | **高** |
| 7 | §2.4 / §2.8 / §7 步骤 0 | 惰性 `Mask` 裁剪**未实现**,两条守门员测试不存在;入库的类型名/字段名与本文提案不一致 | 中 |
| 8 | §8.1 | 步骤 1"1–1.5 人周"与自述的"适配器 150 行"矛盾,兄弟文档同类范围实测 3–5 人日;同时漏了 0b/`catch_unwind`/`Anim` 扩容三项 | 中 |
| 9 | §8.2 第 4 条 | 依赖 crate 数 ≈8 → **实测 15**(漏算 kurbo/peniko/serde_json 的传递依赖) | 中 |
| 10 | §8.2 第 3 条 / §9 第 6 条 | MSRV 构建道"若开 feature"的"若"可以现在消掉:msrv job 跑 `--all-features` | 中 |
| 11 | §9 第 3/4 条 | 已被同批入库的兄弟文档实测回答(15 crate 秒级编译;`append` **0.007ms/帧**) | 中 |
| 12 | §8.3 / §8.4(新增) | 漏掉四条更便宜的路:程序化动效组件、只做 SVG 图标、**build 期预渲染成数据表**、静态首帧独立发布 | 中 |
| 13 | §2.9 / §7 步骤 1 | 漏掉的实现细节:`kurbo::Stroke` 是 `start_cap`+`end_cap` 两个字段、`path_elements` 的 tolerance 未定、根裁剪要特判成 `push_clip` | 低 |
| 14 | §4.1 / §4.4 | 行号漂移:`lib.rs:211/227/278` → 实际 `214(pump 在 220)/230/281` | 低 |

### 10.3 无法验证 / 明确没做的

1. **没有跑任何 lottie。** 本轮复核**未**在本机接 velato:本文是设计文档,
   没有可运行产物;而 `lottie-1-ecology.md` §6 已经在 scratchpad 里跑过一遍,
   复核选择**引用并交叉检验它的数**,而不是重跑一遍。
   **因此 §6.2 那组帧时间(0.99ms@256px 等)复核未独立复现**,标记为
   "转引自兄弟文档的实测",不是复核自测。
2. **没有跑 30k 布局/绘制探针。** 117ms / 311ms / 111ms / 2525ms 四个数
   全部**转引自** `docs/plans/incremental-layout.md` 与 `docs/DESIGN.md`,
   复核只核对了引文与出处的一致性,未重跑 membench。
   (重跑要开 `--release` 跑 30k 探针,量级是十几分钟且结果机器相关。)
3. **§4.4b 的"丢最后一帧"是源码推导,不是实跑复现。**
   推导链的每一环都核过源码(`anim.rs:111–133` 的 `retain_mut` + 收尾写、
   `lib.rs:232` 的短路条件、现有通道经 `update_style`/`set_scroll` 必 bump),
   但因为 Media 通道还不存在,**无法写一条测试把它钉死**。
   建议实现步骤 2 时**先写 `media_last_frame_is_painted` 让它红**,再修。
4. **§6 的"a11y 每帧空转"同样是源码推导。** `push_access_tree` 每帧跑是确定的
   (`lib.rs:318` 在函数尾部无条件),但"全树遍历的实际耗时"没测 ——
   `update_if_active` 在无 AT 时闭包根本不跑,所以这条开销**只在屏幕阅读器激活时**
   出现,而本仓库的 R3 收尾欠账里正好写着"真机朗读冒烟未做"。
5. **`velato_imaging`(本文与兄弟文档都提到它可作适配器模板)复核未查证**
   —— 它是 0.0.1 版本、单一作者,当模板可以,当依据不行。
6. **§8.4 ③(build 期预渲染成数据表)复核未做体积估算。**
   "24px/60 帧很小、全屏插画会爆"是定性判断,没有具体字节数。
   真要立项,先量一个:Tiger.json 60 帧展开成 `PathCmd` 表有多大。
7. **未核实 `.svelte` / `view!` 侧新增 `<lottie>` 元素的实际改动面**(§7 步骤 3)。
   本文按 `<overlay>` 那次的经验估 0.5 人周,复核没有去读
   `sv-compiler` 的 `ElemKind` 表来验证这个类比是否成立。

### 10.4 一句话结论

**上游调研这一半(§1、§5.1)可以直接采信,复核逐条重查零错误。
架构裁决这一半(§2–§4、§6–§8)需要按上表重读**:
它的骨架(不新增 ElementKind、接单一 pump、CPU 也能做、降级到两后端一致)
**是对的**,而且 §4.4 那个 vello 短路是真发现;
但它建立在一个**已经过期的前提**上,把最大的一堵墙(绘制/脏矩形)
留在了视野外,并且把上游的**进程存活性风险**记成了画质风险。
**在脏矩形与 `catch_unwind` 就位之前,不要开步骤 2。**
