...
"3000 **节点** 的建树基线 1–2 ms"——但 3000 个 8 槽位的 `.sv` 组件实例根本不是 3000 个节点。
...
> **验收闸(S2 起)**:membench 增开 `--scene sfc-rows`(行是 `.sv` 组件),
> 卡 `build_ms` 相对拆分前 **≤ +20%**。
...
2 个测试)+ `tests/fixtures/{wide,child,parent}.sv` 与对应 `.rs.expected`
(commit `1a07b06`)。fixture 是 `wide.sv`(铺开语法面)与 `child.sv`+`parent.sv`(组件面),
