...
- **API 冻结顺序**:步骤 2 会给 sv-ui 加公开 API(`take_dirty`)。DESIGN.md R4 把
  "双前端内核合并"列为 API 冻结的最后前置。〔2026-07-23 更新〕该前置已出清
  (内核合并落地),`take_dirty` 的 API 面直接进冻结清单评审即可。脏日志这个 API 面很小、且是
...
**但上面那三个数字只量了布局。整帧口径要小得多,必须一起给** ——
`membench --frames 20 --hold-secs 0`,对照 `examples/membench/README.md` 的基线:

| 场景 | 基线 | 现在 | 倍数 |
|---|---|---|---|
| `--scene scroll --controls 3000 --mutate` | 19.26ms | **11.68ms** | 1.65× |
| `--scene rows --controls 3000 --mutate` | 19.74ms | 17.27ms | 1.14× |
| `--scene rows --controls 3000`(静止) | 15.43ms | 12.48ms | 1.24× |
| `--scene text --controls 2000 --mutate` | 93.71ms | 84.56ms | 1.11× |
| `--scene text --controls 2000`(静止) | 48.07ms | 42.27ms | 1.14× |

**读法**:滚动帧的布局成本确实塌到了 0.6ms,但整帧还要 11.68ms ——
**剩下的 11ms 全是绘制**。所以对外能说的是"滚动/打字/换色不再触发全量布局",
**不能**说"滚动快了 44 倍"。44 倍是布局那一段的比值,不是用户看到的帧。

这同时把 §8.2 那句推断变成了实测支持的结论:**布局已经不是瓶颈了,
下一件该排的事是绘制端(shape 缓存 / 脏矩形)**。
