...
| **(c) 不该做** | rlottie/dotlottie-rs/ThorVG 绑定;自研解析器;文本层与表达式;构建期**转译成 `.svelte`**(≠ 上面的烘焙) | —— |
...
> `sv-compiler/emit.rs:28-49` 的 `ElemKind` 镜像 + `create()`(否则 `.svelte` 用不了),
...
和 §7.3 第 4 条否掉的 "lottie → `.svelte` 代码" **完全不是一回事**,别混:
...
4. **构建期把 lottie 转译成 `.svelte` / `view!` 代码**(Qt 的 lottietoqml 路线)。
...
- **构建期烘焙**(≠ §7.3 否掉的"转译成 `.svelte`"):build.rs 跑 velato 出逐帧
