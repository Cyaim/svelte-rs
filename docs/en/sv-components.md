...
Mixed static/interpolated text compiles to a single `bind_text` binding; fully static text gets zero bindings. Attribute values are `name="static string"` or `name={rust_expr}`. The event set today is `onclick`/`on:click`, `onpointerenter`, `onpointerleave` — anything else (keyboard, input) waits on the focus-chain work and is a compile error, not a silent no-op. Inline `style="k:v; ..."` and shorthand style attributes (`fg=`, `font-size=`, …) are the styling mini-language — see [./styling.md](./styling.md).
