...
- **R2 视口与布局**(调研 22/23):滚动体系(5.5–8.5 人周;裁决:`Style.overflow`
  ...
  **2525ms → 111ms(22.7 倍)**,逐行唯一与共享文本之差从 27 倍收敛到 1.22 倍。
  ...
  —— 全仓 34 个 `bump` 点,人工整理并复核过两轮的分级表仍漏了 8 个);
  渲染壳按级决定重建树 / 只重走 / 整份复用 `Rc<Layout>`。
  实测(约 6000 节点滚动列表,release,**只量布局**):全量 29.05ms、
  滚动帧 0.66ms(44×)、空转/打字/换色帧 0.0008ms(36000×)。
  **整帧口径要小得多,一起给**(membench 对照 README 基线):
  滚动 3k 档 19.26 → **11.68ms(1.65×)**、rows 静止 15.43 → 12.48ms、
  text 变更 93.71 → 84.56ms。读法:滚动帧的布局塌到了 0.6ms,
  但整帧仍要 11.68ms —— **剩下的 11ms 全是绘制**。
