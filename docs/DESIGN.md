...
> "编译器核心独立库 + 双前端(view! 宏 / .svelte 文件)共存"**,不做二选一。

**实证事实**(本仓库,两条路线均全绿):
- `crates/sv-macro`:view! 宏(parse/ir/codegen 分层,12 测试);
- `crates/sv-compiler`:.svelte SFC 编译器(runes 源变换 + 原汁 Svelte 模板语法 +
...
1. rustc 类型错误落在生成文件而非 .svelte(rustc 无 `#line`;实验对比见 10 号报告),
   proc-macro 则 span 精确到用户源码字符。缓解三层:模板域错误编译器自报(已做到,
   带 .svelte 行列)→ 生成代码可读 + 锚点注释 → `sv check` 诊断重映射。
...
2. `.svelte` 内 Rust 表达式无 rust-analyzer:唯一解是 Volar/otter.nvim 式转发(生成文件
   落盘为一等产物 + .map 双向映射 + LSP 转发),MVP 约 9–16 人周,第一年只做只读特性。
   **这是 .svelte 前端能否转正的最大悬置条件**;双前端策略把这笔税从赌注变成选项。
...
**① 第一步(2026-07-22)**:新增 `sv_compiler::emit`
——**绑定原语调用词汇表 + 重建闭包协议**,两个前端唯一的发射口;`view!` 宏
改为依赖 sv-compiler,从同一词汇表发射(15 项宏测试与 55 项编译器测试零改动
通过)。于是"改一个原语签名要同步改两处 codegen"的纪律作废,改一处即可。
当时**刻意没有合并 IR 与解析**,理由是"硬合成一份 IR 会把宏路径的 span 精度
赔进去"——该保留已被下一条推翻(方案绕开了'硬合成')。

**① 完成(2026-07-23,内核合并落地)**:公共模板 IR = `sv_compiler::template`
(转正为 pub),其表达式载荷 `ExprSrc` 做成**双态**而非归一成字符串:
`Text{src,offset}`(`.svelte`,codegen 期经 sourcemap 垫位解析 + runes 改写)|
`Tokens(TokenStream)`(`view!`,带真 span 的 token **原样直通,不过任何改写**
——runes/force-move/普通变量预克隆都是 `.svelte` 表面语法的语义);`Each` 的
模式与索引同样双态(模式的借用错误也指到用户源码)。codegen 从此只有一份
(`Cg`,宏侧入口 `sv_compiler::generate_template`),**两个前端只剩各自 parser**:
`view!` 的 token parser(sv-macro,表面语法校验全在解析期、带真 span)与
`.svelte` 的文本 parser。宏侧 ir.rs/codegen.rs 退役(-343 行)。
**span 精度未被牺牲的证据**(当年保留双前端的理由之一):宏 parse 错误行列
断言 ×5 + 内核 token 直通契约测试 ×7(span 恒等/不过 runes 改写/不强加 move,
经变异验证);`.svelte` 侧 golden 逐字节 + sourcemap 21 项 + 单测零改动,
`view!` 侧 15 项行为测试零改动。属性名表仍留在各前端(表面语法本就不同:
`on_click(闭包)` vs `onclick={闭包}`,宏 parser 映射到共享 IR 的属性词汇)。
与 ③(setup/render 拆分)的关系:③ 的起点从".svelte 专属 codegen"变为
**共享内核**,其"view! 宏不跟 stamp"的预裁决在 ③ 动工时需复议(那时 emit
建树词汇表的消费者结构已不同)。
`.svelte` 表面语法未决点:`$state`(Svelte 保真,token 级预处理,08 实验可行)vs 普通
函数形态(script 100% 合法 Rust,rustfmt/LSP 友好,09 主张)——原定"留到核心
合并时定";**2026-07-23 核心合并落地时明确再推迟**:runes 变换整个位于
`.svelte` parser 分叉内(script.rs),与共享模板内核正交,两种表面语法内核都能
支撑,不构成阻塞;留到 LSP/IDE 路线(转正悬置条件)定型时一并裁决。
...
> **✅ 2026-07-23 脏矩形落地(CPU 呈现路径)**:滚动帧 scroll-blit(搬像素 +
> 只重画露出条),Paint/Measure 帧按 `DirtyItem` 的 `id` 局部重画(Paint 已带
> id),帧号动画(`set_anim_frame` = Paint{id})现在**只重画动画那块矩形**——
> 上一段要的"另一半省电"到手。位图帧动画(Frames)天然受益;**矢量档
> (Lottie)例外**:`fill_path` 墨迹范围无从保守估计,出现即整帧,留给
> shape/路径边界缓存之后再收。守卫与逐字节等价见 `sv-shell/src/damage.rs`
> 模块注释与差分测试;实测(release)滚动场景离屏 12.9→2.2ms/帧、开窗
> 55→~100fps(见 `examples/membench/README.md`)。
...
**代价与边界**:项类型需 `Clone + PartialEq`;`.svelte` 里 keyed 绑定名必须是**单个
...
### ADR-10(2026-07-23,**已裁决:`svelte-rs`**)项目/包名与文件后缀
> 触发条件:crates.io 首发。**必须先于首发定案**——发布后改名等于弃坑重来。
>
> **裁决(2026-07-23,维护者定案)**:
> - **项目名 / MIT 开源名 / 包名 = `svelte-rs`**(已是 GitHub 仓库名,直白说明血缘、SEO 好)。
>   商标风险由维护者承担(取"直说血缘、不蹭 svelte crate 名"的折中);伞 crate
>   `svelte-rs` 作为单一入口 re-export 生态,**子 crate 保持 `sv-*`**(crates.io 空闲、
>   短、tokio/tokio-* 同款分层),不做九 crate 大改名。
> - **SFC 文件后缀 `.sv` → `.svelte`**,与 Svelte 对齐(零心智负担:Svelte 开发者
>   见 `.svelte` 即知其义)。已落地:`sv_compiler::build` 的扩展探测、11 个示例文件、
>   golden fixtures、`sv check` 的 panic-dump scraper(`.svelte:` 匹配)、文档全量;
>   `compile_sv`/`sv-check` 等**标识符名**归 API 冻结批(B)——**已于 2026-07-23 落地**:
>   `compile_sv`/`compile_sv_with`/`compile_sv_mapped` → `compile`/`compile_with`/
>   `compile_mapped`;二进制 `sv-check` → **`cargo-sv` + `check` 子命令**(即
>   `cargo sv check`,逐字吻合调研 06 的设想;刻意不用裸 `sv`——会与 Linux runit
>   的 `/usr/bin/sv` 撞名,cargo-* 前缀无 PATH 遮蔽),诊断尾注/problemMatcher
>   source 同步为 "sv check"。**边界**:`.svmap` 磁盘格式(`svmap 1` 版本头与行键)
>   与 `__sv_*` 内部占位符不动——它们是项目前缀/持久化契约,不是已废弃的 `.sv` 后缀。
...
| sv-compiler | .svelte SFC 编译器前端(sfc/script runes 变换/template/style/codegen,prettyplease 可读产物,错误带 .svelte 行列) | 7 |
...
| examples/counter-sfc | 计数器 · .svelte 编译器路线(build.rs 集成 + 端到端行为测试) | 1 |
...
- **R1 输入地基**(调研 20/21):键盘通道+焦点链+快捷键(4.5–6 人周;裁决:焦点
  ...
  tabindex;.svelte 走 Svelte 5 `onkeydown` 属性形态)
...
- **R2 视口与布局**(调研 22/23):滚动体系(5.5–8.5 人周;裁决:`Style.overflow`
  ...
  `overflow_x`(横),`.svelte` 认 `overflow`/`overflow-x`/`overflow-y`(简写
  写两轴);**只在该轴可滚时给滚动范围** —— 横向 hidden + 纵向 scroll
  的容器不会被滚轮横推、滚动条也只出纵向那根;任一轴非 Visible 就裁剪
  (裁剪矩形是二维的,没法只裁一轴);
  ——**✅ 2026-07-22 S6 平滑滚动落地**:滚轮给的是**目标**,偏移由 anim
  通道逐帧逼近(140ms ease-out);连续滚在**进行中的目标**上累加而不是在
  "这一帧画到哪儿"上累加——后者会让快滚越滚越慢;同一容器重复滚只改目标
  不新开动画(否则叠出一堆互相打架的动画);收尾帧写精确目标(缓动在 t=1
  才严格等于 to,浮点差一点会留半像素错位)。**只给鼠标滚轮补缓动**:
  LineDelta 是离散的一格一格,不补会一跳 40px;触摸板 PixelDelta 本身连续,
  再补一层只会更黏。**留档 B**:惯性(需速度追踪)、触摸滚动(需真机);
  ...
  列入档 B(2–3 人周);
  ——**⚠️ 2026-07-22 纠偏:这个条目的名字和方向都是错的**
  (依据 `docs/plans/incremental-layout.md`,含实测)。
  (1)**增量能力不需要实现 taffy 的低层 trait**:每节点 9 槽缓存 +
  `mark_dirty` 沿父链清缓存,高层 `TaffyTree` 早就全有;我们拿不到增量的
  唯一原因是**每个变更帧把整棵树扔掉重建**。条目应更名为
  "增量布局(持久 TaffyTree + 变更分级)"。
  (2)**瓶颈也不在树重建,在 measure**——而且当时那个 ~130–160ms 的读数
  本身就偏了:探针让 30000 个叶子共享两种文本串,而 measure 缓存的键含文本,
  于是**永远命中**,量到的是"没有 measure 成本的布局"。改成逐行唯一文本
  (真实界面的口径)后是 **2525ms**。
  (3)**✅ 已修掉大头**:`text.rs` 两代缓存的 `CAP = 4096` 装不下 30k 树的
  工作集,一装满就整代降冷、下一轮要的又刚被顶掉 → 缓存颠簸,退化成
  "每叶每帧重排一次"。改成按工作集自适应(钳 [4096, 65536])后
  **2525ms → 111ms(22.7 倍)**,逐行唯一与共享文本之差从 27 倍收敛到 1.22 倍。
  一个常数,不报错,只是慢 20 倍——这类洞只有把量尺先摆正才看得见。
  ——**读数口径**:上面这组 2525/111ms 是 `sv-shell` 探针的树
  (6000 行 × 5 叶,**30000 个各不相同**的短串);`incremental-layout.md`
  里的 "30k 全量 311ms" 是 membench `--scene rows` 的同构树
  (**只有 6000 个** distinct 串,列文本全树复用)。两个都是真数,
  但**不是同一棵树**,不能互相印证也不能互相推翻;引用时必须带口径。
  (4)**✅ 变更分级 + 持久只读布局树已落地**(2026-07-22,方案与实测见
  `docs/plans/incremental-layout.md` §11)。`sv_ui::dirty` 把每次 `bump()`
  分成 Paint / Position / 结构三级(**分级是 `bump` 的必传参数,漏定级是编译错误**
  —— 签名改必传后,sv-ui 内 `.bump(` 调用点现有 42 处;`incremental-layout.md`
  §11 里那张人工分级表是**重构前零参 `bump()` 的快照(34/26 计数)**,已随签名
  漂移,别拿它当当前覆盖率);渲染壳按级决定重建树 / 只重走 / 整份复用 `Rc<Layout>`。
  实测(约 6000 节点滚动列表,release,**只量布局**):全量 29.05ms、
  滚动帧 0.66ms(44×)、空转/打字/换色帧 0.0008ms(36000×)。
  **整帧口径要小得多,以 membench README 那张重测表为准**(2026-07-22 重测,
  绝对值不跨批次引用、只比同批比值):滚动搅动档 20.32 → **12.45ms(1.63×)**;
  rows 静止档基本持平(13.18 → 13.24);text 全 miss 档 93.71 → 88.21ms。
  读法:滚动帧的布局塌到了 0.66ms,但整帧仍要 12.45ms —— **剩下的 12ms 全是绘制**。
  所以能说的是"滚动/打字/换色不再触发全量布局",**不能**说"滚动快了 44 倍"。
  **档 B 的"滚动/打字/换色不再触发全量布局"至此兑现**;
  这也把"布局是瓶颈"翻篇了:**下一件该排的事在绘制端**(shape 缓存/脏矩形)。
  余下的增量 `mark_dirty`(C 类仍整棵重建)与 walk 优化继续列档 B。
  (5)⚠️ **计划里的"叶内 measure memo"被实测推翻:加了缓存反而慢 29%**,
  且中途一度慢 3.8 倍 —— 因为它砍掉的查询量正是下游缓存自适应的驱动力。
  已改成"没到内存上限就原地扩容"与查询次数解耦,并在代码里留了反向注释。
  这条记在这里是因为它是一类通用陷阱:**在一层缓存前面加一层缓存,
  会让后面那层的自适应失效**;以及"基线塌方时量出来的收益,
  不能拿到基线修好之后用"。
...
- **R3 文本栈/无障碍/弹层**(调研 24/25):Parley 0.11 迁移(10–15 人周;前置
  ...
  **✅ 同日 O4/O6 落地**:Popup 内 ArrowDown/Up = 焦点导航(菜单免费
  获得方向键;dispatch_key 导航段)、`<overlay>` .svelte 内建元素
  ...
  (Dialog.svelte $bindable open 居中模态 + 下拉菜单 + @attach 挂 tooltip,
...
- **R4 发布工程 + API 冻结**(调研 25,4–6.5 人周):改名 ADR-10 **先于 crates.io
  ...
  **生成代码在 codegen 侧自带 allow**——含 rustc 的 unused_braces,
  免得 .svelte 用户被自己没写的代码判罚)、MSRV **1.88** 构建道(声明 MSRV 当场逮到两处实事:`is_multiple_of` 是
  ...
  **打包**:examples/showcase 的 cargo-packager 配置 + 三平台
  ...
  事务,工程侧只做到钩子就位);
  **未做**:cargo-semver-checks(需已发布基线才有意义)。
  ——**✅ 2026-07-22 帧调度 ADR-6 落地**:见上文 ADR-6(开窗路径写入攒到帧
  边界统一冲刷,`tick()` 为逃生舱;离屏与测试路径行为不变)。
  ——**✅ 2026-07-23 API 冻结前置全部出清**(一批三件,语义级 breaking 一次
  出清):双前端内核合并(ADR-2 ①,公共 IR + 共享 codegen)+ `on:` 事件语法
  收敛(遗留 `on:click` 移除,只保 Svelte 5 属性形态 `onclick`)+ 标识符改名
  (ADR-10:`compile` 族 + `sv` 二进制)。首发关键路径推进到
  cargo-semver-checks 接基线 → 六 crate 依赖序发 crates.io。
...
1. **`.svelte` 的 IDE 体验**是编译器路线转正的最大悬置(Volar 式转发 LSP 未 spike;
...
- [06 .svelte 构建集成机制(build.rs/OUT_DIR 定案)](research/06-sv-build-integration.md)
- [07 .svelte 的 IDE/LSP 策略(Volar 式转发可行性)](research/07-sv-ide-lsp.md)
- [08 runes 源变换语义与健全性(变换规则 + 拒绝清单)](research/08-sv-runes-transform.md)
- [09 .svelte 格式设计 + 热重载架构(数据面/代码面)](research/09-sv-sfc-format-hotreload.md)
...
- **VAP(腾讯 Video Animation Player)—— ✅ `crates/sv-vap` 已落地,
  且是这条线上唯一在真实素材上验证过的**。
  起因是一批线上买的直播礼物素材:交付物是 `json + mp4`,而卖家说"用 PAG 播放"。
  **那不是 PAG,是 VAP** —— 两者都出自腾讯、都用在直播礼物上、业内口头常混着叫,
  但文件格式毫无关系。实锤:MP4 里有一个类型为 `vapc` 的自定义顶层 box,
  内容与旁车 JSON 一字不差(所以只发 mp4 也能播)。
  **机制**:H.264 没有 alpha,VAP 就把 alpha 当灰度画面并排塞进同一帧 ——
  实测左边 RGB 750×1624、右边 alpha 375×812(**半分辨率**),
  视频 1136×1632(宏块对齐),直通 alpha 不是预乘。
  **验证**:10 个真实素材全部通过配置解析 + 旁车对拍 + 合成,
  且合成结果与一份独立写的 Python 参考实现**逐字节相同**;
  `examples/vap-gift` 端到端跑通(解码 → 合成 → 场景树 → 时间轴 → `draw_image` → PNG)。
  **真实素材当场纠正了一个错误假设**:验证脚本原本断言"alpha 应当同时有透明区
  与不透明区",结果被打脸两次 —— 全屏不透明的背景类素材、以及几乎全透明的
  细长物体都是合法的。**真实素材的形态跨度比直觉大得多。**
  **不做**:H.264 解码(独立重裁决,与平台强相关)、VAPX 融合动画。
