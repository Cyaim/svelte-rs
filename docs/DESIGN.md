...
| sv-shell | winit 窗口 + CPU 渲染(HiDPI、圆角、CJK、命中测试、离屏 PNG);**键盘接入 + 点击设焦 + 默认焦点环 + IME 全流程 + push_clip/pop_clip 双后端 + arboard 剪贴板(R1)**;**滚动体系:overflow 布局/裁剪/滚轮路由/滚动条/virtual_scroll(R2)** | 15 |
