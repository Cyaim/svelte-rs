...
| sv-ui | 场景树 + bind_text/bind_style/if_block/each_block;版本号 + on_mutate 驱动重绘;**焦点链 + 键盘四段路由 + 快捷键注册表(R1,调研 20)** | 6+11 |
...
| sv-shell | winit 窗口 + CPU 渲染(HiDPI、圆角、CJK、命中测试、离屏 PNG);**键盘接入 + 点击设焦 + 默认焦点环(R1)** | 6 |
...
- **R1 输入地基**(调研 20/21):键盘通道+焦点链+快捷键(4.5–6 人周;裁决:焦点
  ...
  验收:TodoMVC 键入新条目 + Tab 遍历 + Enter 提交;IME 事件序列自动化 ~90%,
  中文输入三平台手测清单通过。
...
- **R3 文本栈/无障碍/弹层**(调研 24/25):Parley 0.11 迁移(10–15 人周;前置
  ...
  修正 ADR-3b"只动 shaping 门面"的乐观表述;PlainEditor 外包 TextInput 编辑内核);
  AccessKit(egui PR #2294 形态:懒激活 + Doc 版本号节拍推送,TreeUpdate.focus
  与 R1 焦点链强耦合,树映射纯函数金样测试);弹层体系(8–13 人周;裁决:离散层
