...
| 档位 | 数量 | 占比 | 一句话 |
|---|---|---|---|
| ✅ 已实现 | 24 | 31% | runes 全家($state/.raw、$derived/.by、$effect、$inspect、$props、$bindable)+ 全部块语法({#if}/{#each} 含 keyed+:else/{#key}/{#snippet}/{@render}/{@const}/{@debug})+ 事件/style:/bind: + 组件(props/children/默认值)+ `<style>` scoped 类,均有测试与 showcase 演示 |
| 🚧 部分 | 7 | 9% | $state(变量粒度)、$effect.pre(别名,pre 时序待帧调度)、mount/生命周期等 |
| 📋 已设计未实现 | 7 | 9% | bind: 表单系(待输入组件)、class: 指令、snippet 作为具名 prop 等——设计已闭合 |
| ⏳ 推迟 | 19 | 25% | $inspect.with/.trace、$effect.tracking/.pending、{#await}、transition 家族、bind: 扩展面——理由明确,时机未到 |
| ❌ 明确不做 | 20 | 26% | HTML 宿主域整域({@html}/svelte:window 系/媒体 bind)、遗留语法(slot/$$props/on: 长期)、被更好机制取代者(use:/bind:this→{@attach},stores→signal) |

> 数字为三批落地后的口径(第二/三批的逐项证据见文末[落地复核](#落地复核2026-07-17)
> 与[第三批落地](#第三批落地2026-07-17含综合演示-examplesshowcase)两表,与正文行冲突时以两表为准)。
