...
| 档位 | 数量 | 占比 | 一句话 |
|---|---|---|---|
| ✅ 已实现 | 44 | 57% | runes 全家(含 .raw/.snapshot/.by/.root/.tracking/.pending/.with、$props.id、writable $derived)+ 全部块语法(含 keyed each、{#await}、{#snippet}/{@render}/{@attach}/{@const}/{@debug}、注释)+ 事件(click/hover/键盘/焦点)/style:/class:/bind:(checked+value+组件 prop)+ `<input>` 文本输入(IME/剪贴板,R1)+ 组件模型(props/children/snippet-props/默认值/回调)+ scoped `<style>` + 进场过渡 in:fade + context/mount/tick,均有测试与 showcase 演示 |
| 🚧 部分 | 4 | 5% | $state 深层响应(**设计性差异**:变量粒度,永久文档化)、$effect.pre(pre 阶段已实现,帧管线语义待 ADR-6)、$inspect.trace(重跑打点版,依赖图谱内省待 dev 工具期)、事件面(键盘/焦点已落地 R1:onkeydown/onfocus/onblur/autofocus + 四段路由;oninput 待 TextInput) |
| 📋 已设计未实现 | 0 | 0% | —— |
| ⏳ 推迟 | 9 | 12% | bind:group/尺寸/函数绑定(待控件族与布局测量)、out:/animate:(待 INERT/FLIP)、class 对象语法、{#await} shorthand、await 表达式、svelte:boundary、属性展开——每项都有明确前置条件 |
...
> 2026-07-18 R1 修订:键盘/焦点落地事件面 ⏳→🚧,TextInput/IME/剪贴板落地
> bind:value ⏳→✅)。
> 计划内(❌ 之外)57 项中 ✅+🚧 已覆盖 48 项(84%);剩余 ⏳ 全部有明确的基建前置
> (控件族/INERT/FLIP/布局测量),不是设计空白。

关键读法:
- **可运行核心闭环已成立并扩展**:从 `.svelte` 源文件到点击交互有两个端到端行为测试
...
| 特性 | Svelte 5 语义 | 桌面适用性 | sv 现状 | 结论/计划 |
|---|---|---|---|---|
| 组件标签 `<Foo prop={x} />` | 组件实例化 + props 传递 | 高 | 📋 无代码(元素集只有 view/text/button,`template.rs` 未知标签报错)。09 §6/EBNF 已定:`CompPath` 按 script 作用域解析,`.svelte` 间 import 就是 Rust `use`,签名统一 `Component` trait | 🔄 主线程施工中(组件+$props);M1 主体 |
...
| 内联 `style="k:v; ..."` 迷你样式语言 + 样式简写属性 | Svelte 无此形态;`<style>` 块落地前的临时静态样式通道 | ✅ `compiler/style.rs`(padding/gap/font-size/radius/width/height/direction/bg/fg),错误带 .svelte 行列;`{表达式}` 形式的 style 明确报错留给 `style:` 指令 |
| `view!` 宏前端 | 同一语义的 proc-macro 路线(Rust 原生 if/for 语法,非 Svelte 语法) | ✅ `crates/sv-macro` 12 测试:view/text/button、if/else-if/else、for(含 index)、文本插值、`style(闭包)`、`on_click(闭包)`。ADR-2:与 .svelte 前端共享内核是 M1 合并目标 |
...
| `onclick` 事件属性 | ✅ 唯一事件形态(遗留 `on:` 指令已移除) | `onclick_svelte5_attr` |
...
| `onpointerenter/leave` | ✅(shell 悬停派发) | showcase 行为测试 |
| `onkeydown` / `onfocus` / `onblur` / `autofocus`(R1,调研 20) | ✅ 焦点链 + 五段路由(冒泡→编辑→Tab 导航→Enter/Space 激活→快捷键);onkeydown 自动 set_focusable;onfocus/onblur 合成单一回调;快捷键注册表挂 on_cleanup 自动注销 | sv-ui focus/shortcuts 11 测;`onkeydown_compiles`;宏端 `on_key_down_wires_focus_and_bubbles`;counter-sfc 键盘端到端 |
| `<input>` + `bind:value` / `oninput` / `onsubmit` / `placeholder`(R1,调研 21) | ✅ 单行 TextInput:EditOp 编辑内核(UTF-8 char 边界)、光标/选区/全选、IME 预编辑 over-the-spot(value 不含半成品)、Ctrl+A/C/X/V(arboard,测试注入假剪贴板)、Enter 提交、光标跟随滚移 + push_clip;宏端 `bind_value/on_input/on_submit/placeholder` 同步 | sv-ui input 6 测;shell 金样/caret 互逆/IME 光标区域 3 测;`input_bind_value_two_way`;todo-sfc TodoMVC 键入验收;input-demo 手测台 |
| `overflow` 滚动 + `onscroll` / `bind:scrolly`(R2,调研 22) | ✅ 滚轮 + 最近可滚祖先滚动链、裁剪双后端、滚动条合成绘制、视口外不可点/不可悬停、virtual_scroll 虚拟化桥(100k 行接真实滚轮);bind:scrolly 链式保留用户 onscroll;宏端 `on_scroll/bind_scroll_y` 同步 | shell 滚动 6 测(布局平移/钳制/滚动链/金样/像素级裁剪/虚拟化);`sfc_overflow_and_scroll_bindings_compile`;宏端 `scroll_bindings_roundtrip` |
| `aria-label`(R3,调研 24) | ✅ 无障碍名称覆盖(静态串 + `{表达式}` 响应式);AccessKit 语义树 role/名称/bounds/焦点/动作全量直通,懒激活适配器接三平台 AT;宏端 `aria_label` 同步 | `aria_label_compiles`;`a11y_roles_names_bounds_golden`;`a11y_action_dispatch_roundtrip` |
| `<overlay>` 弹层元素(R3,调研 25;Svelte 无对应——不发明 {#teleport}) | ✅ open/anchor(below/above/left/right/center,锚父容器)/gap/modal/close/ondismiss/style;children 即 build 闭包;离散层+注册序(无 z-index)、越界翻转、click-outside/Esc LIFO、modal 焦点陷阱、Popup 内方向键导航、tooltip 原语(@attach 挂);Dialog.svelte 组件自举($bindable open) | `sv_overlay_codegen`;shell 弹层 5 测;overlay-demo 菜单/对话框端到端 + 离屏人查 |
