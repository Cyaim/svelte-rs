...
| 📋 已设计未实现 | 0 | 0% | —— |
| ⏳ 推迟 | 10 | 13% | bind:value/group/尺寸/函数绑定(待输入控件与布局测量)、out:/animate:(待 INERT/FLIP)、class 对象语法、{#await} shorthand、await 表达式、svelte:boundary、属性展开——每项都有明确前置条件 |
...
> 2026-07-18 R1 键盘/焦点落地后事件面由 ⏳ 升 🚧)。
> 计划内(❌ 之外)57 项中 ✅+🚧 已覆盖 47 项(82%);剩余 ⏳ 全部有明确的基建前置
> (输入控件/INERT/FLIP/布局测量),不是设计空白。
...
| `onkeydown` / `onfocus` / `onblur` / `autofocus`(R1,调研 20) | ✅ 焦点链 + 四段路由(冒泡→Tab 导航→Enter/Space 激活→快捷键);onkeydown 自动 set_focusable;onfocus/onblur 合成单一回调;快捷键注册表挂 on_cleanup 自动注销 | sv-ui focus/shortcuts 11 测;`onkeydown_compiles`;宏端 `on_key_down_wires_focus_and_bubbles`;counter-sfc 键盘端到端 |
